...
def main() -> int:
    ...
    llm = LLM(
        model=BASE, enable_lora=True, max_loras=4, max_lora_rank=32, dtype="bfloat16", gpu_memory_utilization=0.85
    )
    ...
    if base_out == a1:
        print("FAIL: base output == LoRA A output (adapter not applied)")
        return 1
    if a1 == b1:
        print("FAIL: LoRA A output == LoRA B output (swap not effective)")
        return 1
    if a1 != a2:
        print("FAIL: same LoRA + greedy + same seed produced different output (nondeterminism)")
        return 1
