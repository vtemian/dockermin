...
def _verify(triple: dict, new_df: str) -> tuple[int | None, bool, str]:
...
def _entry(name: str, triple: dict, new_df: str, t0: float) -> EvalEntry:
...
def _error_entry(name: str, triple: dict, t0: float, err: str) -> EvalEntry:
    return EvalEntry(
        baseline=name,
        triple_id=_triple_id(triple),
        new_size_bytes=None,
...
def _vllm_generate(messages: list[dict], max_tokens: int = 1024, temperature: float = 0.2) -> str:
...
def baseline_qwen_zero_shot(triple: dict) -> EvalEntry:
...
def _fix_dl3009(df: str) -> str:
    ...
    out = []
    for line in lines:
        if re.search(r"\bapt-get\s+install\b", line) and "/var/lib/apt/lists" not in line:
            stripped = line.rstrip()
            # Strip a trailing backslash if present, append the cleanup, restore the backslash.
            if stripped.endswith("\\"):
                core = stripped[:-1].rstrip()
                out.append(f"{core} && rm -rf /var/lib/apt/lists/* \\")
            else:
...
def _fix_dl3060(df: str) -> str:
    ...
    out = []
    for line in lines:
        if re.search(r"\byarn\s+install\b", line) and "yarn cache clean" not in line:
            stripped = line.rstrip()
            if stripped.endswith("\\"):
                core = stripped[:-1].rstrip()
                out.append(f"{core} && yarn cache clean \\")
            else:
                out.append(f"{stripped} && yarn cache clean")
        else:
            out.append(line)
    return "\n".join(out)
...
def _run_hadolint(df_text: str) -> list[dict]:
...
def baseline_hadolint(triple: dict) -> EvalEntry:
...
def baseline_slim(triple: dict) -> EvalEntry:
    ...
    try:
        ...
        # 2) Invoke slim build. http-probe disabled (no http target); continue
        # after 15s steady-state.
        ...
        if proc.returncode != 0:
            return _error_entry("slim", triple, t0,
                                f"slim build failed rc={proc.returncode}: {proc.stderr[-300:]}")

        # 3) Look for a slim-emitted Dockerfile.
        slim_df_path = workdir / ".slim-state" / original_tag / "Dockerfile"
        if slim_df_path.exists():
            new_df = slim_df_path.read_text()
...
def _collapse_consecutive_runs(df: str) -> str:
    ...
    while i < len(lines):
        ...
        if not m:
            ...
            continue
        indent = m.group(1)
        # Collect the body of this RUN (handling line continuations) and any
        # subsequent RUN statements.
        body_parts: list[str] = [m.group(2)]
        i += 1
        # Continue any trailing-backslash continuations of the current RUN.
        ...
        while i < len(lines):
            m2 = re.match(r"^\s*RUN\s+(.*)$", lines[i])
            if not m2:
                break
            next_body = [m2.group(1)]
            i += 1
            while next_body[-1].rstrip().endswith("\\") and i < len(lines):
                next_body[-1] = next_body[-1].rstrip()[:-1].rstrip()
                next_body.append(lines[i].lstrip())
                i += 1
            body_parts.append("&& " + next_body[0])
            body_parts.extend(next_body[1:])
        if len(body_parts) == 1:
            out.append(f"{indent}RUN {body_parts[0]}")
        else:
            joined = " \\\n    ".join(body_parts)
            out.append(f"{indent}RUN {joined}")
    return "\n".join(out)


def baseline_manual_best_practice(triple: dict) -> EvalEntry:
...
def _hf_dockermin(model_id: str):
...
def baseline_dockermin(triple: dict, model_id: str = "vladtemian/dockermin-qwen7b-lora-v1") -> EvalEntry:
...
def run_one(baseline_name: str, triple: dict, **kwargs) -> EvalEntry:
